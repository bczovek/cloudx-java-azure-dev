...
class HealthMonitor {
  ...
  startMonitoring() {
    ...
    console.log(
      `Health monitoring started (interval: ${this.options.checkInterval}ms)`,
    );
  }
  ...
  handleHealthResponse(data) {
    ...
    if (overallStatus === "UP" && downServices.length === 0) {
      ...
      const downServiceNames = downServices.map((s) =>
        this.getServiceDisplayName(s.name),
      );
  ...
  handleHealthError(error) {
    ...
    if (this.retryCount < this.options.maxRetries) {
      ...
      this.updateStatus(
        "checking",
        `Retrying... (${this.retryCount}/${this.options.maxRetries})`,
        "fa-sync fa-spin",
      );
...
window.debugHealth = {
  ...
  status: () =>
    console.log("Health monitor:", window.healthMonitor?.getCurrentStatus()),
